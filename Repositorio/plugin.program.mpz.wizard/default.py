...
def index():
	...
	addDir ('Wizards'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count18 > 0:
					...
					addFile('[B]%s Wizards para Kodi 18 Leia (%s)[/B]' % (state, count18), 'togglesetting',  'show18', themeit=THEME3)
					if SHOW18 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s Wizards para Kodi 17 Krypton (%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count16 > 0:
					...
					addFile('[B]%s Wizards para Kodi 16 Jarvis (%s)[/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        addFile('[COLOR lime][B]INSTALAR WIZARD[/B][/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
        addFile('', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def maintMenu(view=None):
    ...
    feq        = ['Siempre', 'Diario', 'Cada 3 dias', 'Semanal']
	...
	addDir ('[I]<< Volver al Menu Principal MPZ[/I]', icon=ICONMAINT, themeit=THEME2)
	setView('files', 'viewType')
...
def changeFeq():
    feq        = ['En cada arranque', 'Diario', 'Cada 3 dias', 'Semanal']
